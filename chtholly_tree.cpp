...
struct Range {
  ...
  Range(long long left, long long right = -1, long long val = 0)
...
class ChthollyTree {
 public:
  ...
  void Insert(int l, int r, int v) { range_set_.emplace(l, r, v); }
  ...
  long long Sum(long long l, long long r, long long exp, long long mod) {
    ...
    for (; iter_l != iter_r; iter_l++) {
      res =
          (res + (iter_l->r - iter_l->l + 1) * pow(iter_l->v, exp, mod)) % mod;
    }
    return res;
  }
