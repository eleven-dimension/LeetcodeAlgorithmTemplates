...
std::optional<long long> FindFirst(long long l, long long r, auto condition, BoundType bound_type) {
    if (r < l) {
        return std::nullopt;
    }
    ...
    if (bound_type == BoundType::Lower) {
        iter = std::ranges::lower_bound(range, 0, [&](long long x, auto) {
            return condition(x);
        });
    } else {
        iter = std::ranges::upper_bound(range, 0, [&](auto, long long x) {
            return condition(x);
        });
    }
